...
## 2024-07-08 - Keyboard Focus Support for Tooltips
**Learning:** Added `group-focus-visible:visible group-focus-visible:opacity-100` alongside `group-hover:visible` for element tooltips so they are accessible via keyboard navigation. Paired it with `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-md` on the container to match repository conventions.
**Action:** Always include keyboard focus equivalents (`group-focus-visible`) when implementing hover-triggered tooltips or visibility changes.
